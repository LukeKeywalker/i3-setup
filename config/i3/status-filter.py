...
def _convert_usages(usages, label):
    """Convert the usage string to unicode block characters."""
    return label + "".join([_usage_block(int(x.strip(_STRIP_CHARS)) / 100) for x in usages.split()]) + _PADDING
 

def _scale_load_to_percents(load_values):
    max_load = 10.0
    result = ""
    for load_value in load_values.split():
        usage_float = min(float(load_value), max_load)
        result += str(round(usage_float / max_load * 100.0)) + " "
    return result
...
def _process_element(element):
    ...
    if element['name'] == "cpu_usage":
        element['full_text'] = _convert_usages(element['full_text'], "CPU CORES: ")
    elif element['name'] == "load":
        element['full_text'] = _convert_usages(_scale_load_to_percents(element['full_text']), "SYS LOAD: ")
    return element
